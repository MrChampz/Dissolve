...
BlurFilter::BlurFilter(ID3D12Device* device, UINT width, UINT height, DXGI_FORMAT format)
{
	...
	// Pre-calculate the Gauss weights.
	...
	int blurCount)
{
	...
	cmdList->SetComputeRoot32BitConstants(3, 1, &blurRadius, 0);
	cmdList->SetComputeRoot32BitConstants(3, (UINT)weights.size(), weights.data(), 1);

	cmdList->ResourceBarrier(1,&CD3DX12_RESOURCE_BARRIER::Transition(
		input, D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_SOURCE));

	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		m_BlurMap0.Get(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_COPY_DEST));
	...
	for (int i = 0; i < blurCount; ++i)
	{
		...
		cmdList->SetComputeRootDescriptorTable(0, m_Blur0GpuSrv);
...
CD3DX12_GPU_DESCRIPTOR_HANDLE BlurFilter::GetOutputSrv()
{
	return m_Blur1GpuSrv;
}

void BlurFilter::CalcGaussWeights(float sigma)
{
